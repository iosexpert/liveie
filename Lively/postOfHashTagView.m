...
@interface postOfHashTagView ()
{
    NSMutableArray *posts;
    UIImageView *heartanimation;
    int cellToLoad,options;
...
@implementation postOfHashTagView
...
- (void)viewDidLoad {
    ...
    if(postArr==nil)
    {
        lblTop.text=selectedHashTag;
     
        
        NSDictionary *params;
        params=@{
                 @"hashtag"          :[selectedHashTag stringByReplacingOccurrencesOfString:@"#" withString: @""],
                 @"limit"            :@"12",
                 @"pageno"           :@"0",
                 @"userid"          :userid,
                };
        ...
        if (!paramError) {
            ...
            [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
                ...
                if (JSONdata != nil) {
                    ...
                    [self searchNearesttextResult1:JSON];
                ...
                NSLog(@"Network-Response: HTTP-Status Code: %ld, Error: %@", (long)[httpResponse statusCode],
                      error);
                [self searchNearesttextResult1:nil];
    ...
    else
    {
        ...
        [hashtagScroll addSubview:imgSearchBack];
        [hashtagScroll addSubview:searchBaar];
        [hashtagScroll setContentOffset:CGPointMake(0, 60) animated:YES];
        
        NSString *st=[NSString stringWithFormat:@"%@/%@/%@",userid,@"0",@"12"];
        lblTop.text=strTop;

        
        NSString *urlString=[NSString stringWithFormat:@"%@/Post.svc/GetRecentPost/%@",appUrl,st];
        ...
        [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            if (JSONdata != nil) {
                ...
                [self searchNearesttextResult2:JSON];
            ...
            [self searchNearesttextResult2:nil];
...
-(void)getShowMoreData
{
    ...
    if(apicall==1)
    {
    if([lblTop.text isEqualToString:@"RECENT"])
    {
        ...
        NSLog(@"url %@",urlString);
        NSURL *urlk = [[NSURL alloc] initWithString:urlString];
        NSURLRequest *request = [[NSURLRequest alloc] initWithURL:urlk];
        AFHTTPRequestOperation *operation = [[AFHTTPRequestOperation alloc] initWithRequest:request];
    ...
    else
    {
        ...
        params=@{
                 @"hashtag"          :[selectedHashTag stringByReplacingOccurrencesOfString:@"#" withString: @""],
                 @"limit"            :@"12",
                 @"pageno"           :[NSString stringWithFormat:@"%d",(int)postArr.count],
                 @"userid"          :userid,
                 };
        ...
        [api_obj postOfHashTags:@selector(getShowMoreDataResult1:) tempTarget:self :params];
    }
    }
}
    ...
    else
    {
        if([[[dict_Response objectForKey:@"response"]valueForKey:@"status" ] integerValue]==200)
        {
            [indicator removeFromSuperview];
                ...
                if([[dict_Response valueForKey:@"recent"] count]>0)
                {
                for(int i=0;i<[[dict_Response valueForKey:@"recent"] count];i++)
                {
                    [postArr addObject:[dict_Response valueForKey:@"recent"][i]];
                    [posts  addObject:[dict_Response valueForKey:@"recent"][i]];
                }
                ...
                else
                {
                    ...
                    CGRect labelFrame = CGRectMake (0, collection_obj.contentSize.height+collection_obj.frame.origin.y-10,self.view.frame.size.width, 20);
                    ...
                    [hashtagScroll addSubview:label];
...
-(void)getShowMoreDataResult1:(NSDictionary *)dict_Response
{
    NSLog(@"%@",dict_Response);
    [LoaderViewController remove:self.view animated:YES];
    if (dict_Response==NULL)
        ...
        if([[[dict_Response objectForKey:@"response"]valueForKey:@"status" ] integerValue]==200)
        {
                ...
                if([[dict_Response valueForKey:@"posts"] count]>0)
                {
                    for(int i=0;i<[[dict_Response valueForKey:@"posts"] count];i++)
                    {
                        ...
                        [ posts  addObject:[dict_Response valueForKey:@"posts"][i]];
                    }
                    for(int i=(int)postArr.count-1;i>=0;i--)
                    {
                        apicall=1;
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(scrollTableOnNotification) name:@"hashtag" object:nil];
    duration=0;
    currentTime=0;
...
-(void)searchNearesttextResult1:(NSDictionary *)dict_Response
{
    NSLog(@"%@",dict_Response);
    [LoaderViewController remove:self.view animated:YES];
    ...
    else
    {
        ...
        if([[[dict_Response objectForKey:@"response"]valueForKey:@"status" ] integerValue]==200)
        {
            ...
            posts = [[dict_Response objectForKey:@"posts"]mutableCopy];;
            //postImages
...
-(void)searchNearesttextResult2:(NSDictionary *)dict_Response
{
    NSLog(@"%@",dict_Response);
    [LoaderViewController remove:self.view animated:YES];
    ...
    else
    {
        ...
        if([[[dict_Response objectForKey:@"response"]valueForKey:@"status" ] integerValue]==200)
        {
            ...
            posts = [[dict_Response objectForKey:@"recent"]mutableCopy];;
            //postImages
            
            
            postArr=[NSMutableArray new];
            postArr=[[dict_Response valueForKey:@"recent"]mutableCopy];
            //[tablev reloadData];
            downloadVideoCount=(int)postArr.count;
            downloadImageCount=(int)postArr.count;
            [self btnImagesClick:0];
...
-(void)showDataAfterApi
{
        ...
        for(int i=0;i<posts.count;i++)
        {
            ...
            if(iphone5 || iphone4)
            {
                ...
                [hashtagScroll addSubview:img1];
                
                
                UIButton *button = [UIButton buttonWithType:UIButtonTypeRoundedRect];
                [button addTarget:self action:@selector(openHashBy:)forControlEvents:UIControlEventTouchUpInside];
                ...
                [hashtagScroll addSubview:button];
        ...
        hashtagScroll.contentSize=CGSizeMake(self.view.frame.size.width, y+200);
    
    
}

-(void)openHashBy:(UIButton*)btn
{
...
//    {
        ...
        CGRect labelFrame = CGRectMake (0, cell.ImgView.frame.size.height+8,cell.frame.size.width, 45);
        UILabel *label = [[UILabel alloc] initWithFrame:labelFrame];
        [label setFont:myFont];
        label.tag=98;
        label.lineBreakMode=NSLineBreakByWordWrapping;
    ...
    if(indexPath.row==posts.count-1)
    {
        ...
        if(f.size.height<=self.view.frame.size.height+20)
            hashtagScroll.contentSize=CGSizeMake(300, self.view.frame.size.height);
        else
            hashtagScroll.contentSize=CGSizeMake(300, collection_obj.contentSize.height+80);
        collection_obj.scrollEnabled=false;
...
-(IBAction)listGridButton:(id)sender;
{
    ...
    else
    {
        ...
        [scrv setContentOffset:CGPointMake(self.view.frame.size.width+1, 0) animated:YES];
        gridListbtn.tag=1;
        [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(scrollTableOnNotification) userInfo:nil repeats:NO];
//        [self scrollTableOnNotification];
...
-(void)playFullScreenVideo:(UIButton*)sender
{
    ...
    //    duration=0;
    //    currentTime=0;
    //    aTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(aTimee) userInfo:nil repeats:YES];
    //
    //    foooooo=[[dict valueForKey:@"filter"] componentsSeparatedByString:@"$"];
    //
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    {
        api_obj=[[APIViewController alloc]init];
        [api_obj RecentSearch:@selector(searchresult:) tempTarget:self :searchText];
   ...
   if(tableView.contentOffset.y>100)
   {
    ...
    if(section==postArr.count)
    {
        if(!stopLoader)
        {
            ...
            min=[NSString stringWithFormat:@"%d",postArr.count];
...
-(void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    ...
    {
        float scrollViewHeight = hashtagScroll.frame.size.height;
        float scrollContentSizeHeight = hashtagScroll.contentSize.height;
        float scrollOffset = hashtagScroll.contentOffset.y;
        ...
        else if (scrollOffset + scrollViewHeight == scrollContentSizeHeight)
        {
            if(!stopLoader)
            {
                [indicator removeFromSuperview];
                indicator = [[UIActivityIndicatorView alloc]initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
                indicator.frame = CGRectMake(self.view.frame.size.width/2-20, scrollContentSizeHeight-40, 40.0, 40.0);
                [hashtagScroll addSubview:indicator];
                [indicator bringSubviewToFront:scrolv];
                [indicator startAnimating];
    ...
    if([[playableDict valueForKey:[NSString stringWithFormat:@"%d",currentplayingVideo]]isEqualToString:@"YES"])
    {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^(void) {
            ...
            dispatch_sync(dispatch_get_main_queue(), ^(void) {
                ...
                [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playerItemDidReachEnd:) name:AVPlayerItemDidPlayToEndTimeNotification object:[playerr currentItem]];
                }
            });
        });
        if(play == 1)
