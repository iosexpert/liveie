...
- (void)setUpUI {
    CGRect f = self.frame;
    CGFloat width = [UIApplication sharedApplication].keyWindow.bounds.size.width;
    CGFloat height = isIOS7? 30: f.size.height;
    self.frame = CGRectMake(f.origin.x, -height, width, height-10);
    
    CGRect cvF = self.containerView.frame;
    self.containerView.frame = CGRectMake(cvF.origin.x+60, cvF.origin.y-10, self.frame.size.width, cvF.size.height);
