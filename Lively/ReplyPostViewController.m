...
@interface ReplyPostViewController ()
{
    int play;
    int cellToLoad;
...
{
    apiCall=0;
    NSString *urls=[NSString stringWithFormat:@"%@/%@/%@/%@",userid,[selectedFeed1 objectForKey:@"postid"],@"0",@"50"];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    play = 1;
    screenName=@"post";
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(scrollTableOnNotification) name:@"post" object:nil];
    screencount=3;
...
-(void)getWithAndagaistResult:(NSDictionary *)dict_Response
{
    ...
    else
    {
        if([[[dict_Response objectForKey:@"response"]valueForKey:@"status" ] integerValue]==200)
        {
            ...
            [[withPostArr objectAtIndex:0] setValue:@"000000000000000000000000" forKey:@"parent_id"];
            [[withPostArr objectAtIndex:0] setValue:@"public" forKey:@"post_type"];
            [[withPostArr objectAtIndex:0] setValue:@"" forKey:@"action_text"];
            //selectedFeed1=[[dict_Response valueForKey:@"post"]mutableCopy];
...
- (void)edit_action:(UIButton*)btn
{
    ...
    if([[selectedFeed1 valueForKey:@"userid"] isEqualToString:userid] && [[commentFeed valueForKey:@"userid"]isEqualToString:userid])
    {
    ...
    else if([[selectedFeed1 valueForKey:@"userid"] isEqualToString:userid] && ![[commentFeed valueForKey:@"userid"]isEqualToString:userid])
    {
...
- (void)handleSwipe:(UISwipeGestureRecognizer *)swipe {
    ...
    if (swipe.direction == UISwipeGestureRecognizerDirectionLeft) {
        ...
        pushback=0;
        selectedFeed1=[videoPlayDict mutableCopy];;
        recordVideoViewController *mvc;
        if(iphone4)
        {
            mvc=[[recordVideoViewController alloc]initWithNibName:@"recordVideoViewController@4" bundle:nil];
    ...
    if (swipe.direction == UISwipeGestureRecognizerDirectionUp) {
        ...
        {
            selectedFeed1=[videoPlayDict mutableCopy];
            [selectedFeed1 setValue:[selectedFeed1 valueForKey:@"parent_id"] forKey:@"postid"];
        }
        reSharePostViewController *mvc;
        if(iphone4)
        {
...
-(void)playFirstFullscreen:(UIButton*)sender
{
    ...
    if(![str isEqualToString:@""])
    {
        ...
        if(fileExists)
        {
            ...
            //selectedFeed1=[withPostArr[sender.tag]mutableCopy];
...
- (void)Options:(UIButton*)sender
{
    ...
    if(viewOptions.frame.origin.y==790)
    {
        [UIView beginAnimations:@"animate" context:nil];
...
-(void)shareDirectLink:(UIButton*)sender
{
    ...
    selectedFeed1=[dict mutableCopy];
    directVideoViewController *mvc;
    if(iphone4)
    {
        mvc=[[directVideoViewController alloc]initWithNibName:@"directVideoViewController@4" bundle:nil];
...
-(void)followPost:(id)sender
{
    ...
    dict=[withPostArr objectAtIndex:selectbtn];
    api_obj=[[APIViewController alloc]init];
    [api_obj followUser:@selector(followPostResult:) tempTarget:self :[selectedFeed1 valueForKey:@"postid"] : @"true"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section==0)
    {
        ...
        Homecelll = (homeFeedsCell *)[tableView dequeueReusableCellWithIdentifier:simpleTableIdentifier];
        if (Homecelll == nil)
        { NSArray *nib;
            if(iphone6)
            ...
            Homecelll = [nib objectAtIndex:0];
        }
        
        Homecelll.activity.hidden=YES;
        
        Homecelll.videoView.tag=5000+indexPath.section;
        
        
        Homecelll.videoImage.contentMode = UIViewContentModeScaleAspectFill;
        [Homecelll.videoImage.layer setMasksToBounds:YES];
        Homecelll.videoImage.imageURL=[NSURL URLWithString:[NSString stringWithFormat:@"%@",[dict valueForKey:@"thumb"]]];
        
        
        Homecelll.activity.animationImages = [[NSArray alloc] initWithObjects:
                                          ...
                                          nil];
        [Homecelll.activity startAnimating];
        Homecelll.activity.tag=indexPath.section;
        Homecelll.activity.hidden=YES;
        
        
        Homecelll.profiledescription.detectionBlock = ^(STTweetHotWord hotWord, NSString *string, NSString *protocol, NSRange range) {
        ...
        [Homecelll.likeCountBtn setTitle:[NSString stringWithFormat:@"%@",[dict valueForKey:@"likes"]] forState:UIControlStateNormal];
        Homecelll.likeCountBtn.tag=6700+indexPath.section;
        [Homecelll.likeCountBtn addTarget:self action:@selector(Likes:) forControlEvents:UIControlEventTouchUpInside];
        
        
        [Homecelll.likeButton addTarget:self action:@selector(likeActionOfReply:) forControlEvents:UIControlEventTouchUpInside];
        Homecelll.likeButton.tag=indexPath.section+7000;
        if([[dict valueForKey:@"like_status"]integerValue]==1)
        {
            [Homecelll.likeButton setImage:[UIImage imageNamed:@"like_filled"] forState:UIControlStateNormal];
        }
        else
        {
            [Homecelll.likeButton setImage:[UIImage imageNamed:@"like"] forState:UIControlStateNormal];
        }
        
        
        
        
        [Homecelll.commentCount setTitle:[NSString stringWithFormat:@"%@",[dict valueForKey:@"comments"]] forState:UIControlStateNormal];
        
        
        [Homecelll.commentButton addTarget:self action:@selector(comment_action:) forControlEvents:UIControlEventTouchUpInside];
        Homecelll.commentButton.tag=indexPath.section;
        
        
        Homecelll.viewLabel.tag=5600+indexPath.section;
        [Homecelll.viewLabel setTitle:[NSString stringWithFormat:@"%d",[[dict valueForKey:@"views"]intValue] ] forState:UIControlStateNormal];
        
        
                [Homecelll.replyCountButton setTitle:[NSString stringWithFormat:@"%d",[[dict valueForKey:@"reply_count"]intValue] ] forState:UIControlStateNormal];
        
        
        
        
        
                [Homecelll.replyButton addTarget:self action:@selector(reShareButton_action:) forControlEvents:UIControlEventTouchUpInside];
        
        
        if([[Homecelll.replyCountButton titleForState:UIControlStateNormal]integerValue]==0)
            [Homecelll.replyCountButton setTitle:@"" forState:UIControlStateNormal];
        
        
        [Homecelll.optionBtn addTarget:self action:@selector(Options:) forControlEvents:UIControlEventTouchUpInside];
        Homecelll.optionBtn.tag=indexPath.section;
        ...
        [Homecelll.fotterView addSubview:fullScreen];
        ...
        if([[Homecelll.likeCountBtn titleForState:UIControlStateNormal]integerValue]==0)
            [Homecelll.likeCountBtn setTitle:@"" forState:UIControlStateNormal];
        if([[Homecelll.commentCount titleForState:UIControlStateNormal]integerValue]==0)
            [Homecelll.commentCount setTitle:@"" forState:UIControlStateNormal];
        if([[Homecelll.viewLabel titleForState:UIControlStateNormal]integerValue]==0)
            [Homecelll.viewLabel setTitle:@"" forState:UIControlStateNormal];
        if([[Homecelll.viewLabel titleForState:UIControlStateNormal]integerValue]>1000)
        {
            int k= (int) [[Homecelll.viewLabel titleForState:UIControlStateNormal]integerValue]/1000;
            [Homecelll.viewLabel setTitle:[NSString stringWithFormat:@"%dK+",k] forState:UIControlStateNormal];
        ...
        [Homecelll addSubview:viewSep];
        Homecelll.tag=indexPath.section;
    ...
    {
        static NSString *simpleTableIdentifier = @"Fish";
        celll = (ReplyPostTableViewCell *)[tableView dequeueReusableCellWithIdentifier:simpleTableIdentifier];
        if (celll == nil)
        { NSArray *nib;
            if(iphone6)
                nib = [[NSBundle mainBundle] loadNibNamed:@"ReplyPostTableViewCell@6" owner:self options:nil];
            else if(iphone6p)
                nib = [[NSBundle mainBundle] loadNibNamed:@"ReplyPostTableViewCell@6p" owner:self options:nil];
            else if(iphone5||iphone4)
                nib = [[NSBundle mainBundle] loadNibNamed:@"ReplyPostTableViewCell" owner:self options:nil];
