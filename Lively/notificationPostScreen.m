...
@interface notificationPostScreen ()
{
...
@implementation notificationPostScreen
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    NSString *urls;
    if([[selectedFeed objectForKey:@"parent_postid"] isEqualToString:@""])
    {
       urls=[NSString stringWithFormat:@"%@/%@/%@/%@",userid,[selectedFeed objectForKey:@"postid"],@"0",@"50"];
        matchId=[selectedFeed objectForKey:@"postid"];
    }
    else
    {
...
-(void)getWithAndagaistResult:(NSDictionary *)dict_Response
{
    ...
    else
    {
        if([[[dict_Response objectForKey:@"response"]valueForKey:@"status" ] integerValue]==200)
        {
            ...
            {
                if([matchId isEqualToString:[withPostArr[i] valueForKey:@"postid"]])
                {
                    matchidCount=i;
                }
            ...
            [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(tableViewScrollOnPerticularcell:) userInfo:nil repeats:NO];
            ...
            [storyArray addObject:selectedFeed];
...
-(void)tableViewScrollOnPerticularcell:(NSTimer *)timer {
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:matchidCount];
...
- (void)edit_action:(UIButton*)btn
{
    ...
    selectedFeed=[[withPostArr objectAtIndex:0]mutableCopy];
...
-(void)followPost:(id)sender
{
    ...
    dict=[withPostArr objectAtIndex:selectbtn];
    api_obj=[[APIViewController alloc]init];
    [api_obj followUser:@selector(followPostResult:) tempTarget:self :[selectedFeed valueForKey:@"postid"] : @"true"];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if(section == withPostArr.count)
    {
        ...
        label.backgroundColor=[UIColor clearColor];
        [label setText:[NSString stringWithFormat:@"%@",@"No More Post"]];
