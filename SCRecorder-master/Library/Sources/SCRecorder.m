...
static const float kExposureDurationPower = 4; // Higher numbers will give the slider more sensitivity at shorter durations
...
- (void)_applyPointOfInterest:(CGPoint)point continuousMode:(BOOL)continuousMode {
    ...
    double p = pow( 0.5, kExposureDurationPower ); // Apply power function to expand slider's low-end range
    double minDurationSeconds = MAX( CMTimeGetSeconds( device.activeFormat.minExposureDuration ), kExposureMinimumDuration );
    double maxDurationSeconds = CMTimeGetSeconds( device.activeFormat.maxExposureDuration );
...
- (void)setVideoZoomFactor:(CGFloat)videoZoomFactor {
    ...
    if ([device respondsToSelector:@selector(videoZoomFactor)]) {
        NSError *error;
        if ([device lockForConfiguration:&error]) {
            if (videoZoomFactor <= device.activeFormat.videoMaxZoomFactor && videoZoomFactor >=1) {
